...
export default async function CookiesPage() {
  ...
  const row = pool
    ? (await pool.query(`SELECT title, content FROM runpsy_pages WHERE slug = 'cookies'`)).rows[0]
    : null;
  ...
  return (
    <div className="mx-auto w-full max-w-4xl px-6 py-10">
      ...
      <h1 className="font-serif text-4xl mt-4">{row?.title ?? "Настройки файлов Cookie"}</h1>
      <div
